...
const SignUpForm = () => {
    ...
    const password_pattern = "^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!@$%^&*-]).{8,32}$";
    const validateInput = e => {
        ...
        setError(prevState => {
            ...
            switch (name) {
                case "username":
                    if (!value.match(username_pattern) && value) {
                        stateObj["username"] = "Username doesn't match pattern";
                    }
                    break;
                case "password":
                    if (!value.match(password_pattern) && value) {
